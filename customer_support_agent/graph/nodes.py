...
def make_summarize_node(llm) -> Callable[[AgentState], dict]:
    def summarize_and_draft(state: AgentState) -> dict:
        ...
        prompt = (
            ...
            "amount -- else null. If the customer gives an exact figure, use it "
            "exactly. If they describe it as 'more than X' or 'over X', you MUST "
            "use a number strictly greater than X (e.g. X + 1), never X itself -- "
            "X alone does not reflect what 'more than X' means and would be "
            "extracted incorrectly. If they say 'at least X' or 'X or more', X "
            "itself is fine. If the amount is described only vaguely with no "
            "usable figure at all (e.g. 'a large claim', no number given), set "
            "claimed_amount to null AND set needs_escalation to true instead, "
            "since a human should judge an amount that can't be confidently "
            "extracted rather than have it silently default to null.\n"
